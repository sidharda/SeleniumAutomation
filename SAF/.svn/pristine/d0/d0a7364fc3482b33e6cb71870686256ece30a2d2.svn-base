...
public class HTMLReporter extends TestBase implements IReporter
{
    ...
    public void generateReport(List<XmlSuite> arg0, List<ISuite> suites, String outdir)
    {
        try
        {
            outputDir = configProp.getProperty("app.test.reports.directory");
        } catch (Exception ex)
        {
            ex.printStackTrace();
        }
        ...
        try
        {
            modulesCommaSeparated = configProp.getProperty("app.application.test.modules").replaceAll("\\s+", "");
            testGroupsCommaSeparated = configProp.getProperty("app.application.test.groups").replaceAll("\\s+", "");
        } catch (Exception ex)
        {
            ex.printStackTrace();
        }
                            ...
                            if (grp.equalsIgnoreCase(modulename))
                            {
                                ...
                                while (statusIterator.hasNext())
                                {
                                    ...
                                    while (it2.hasNext())
                                    {
                                        ...
                                        try
                                        {
                                            if ((configProp.getProperty("app.enable.logs.in.report")
            ...
            Throwable exception, ITestNGMethod method, String nodeIp, ITestResult result) throws IOException
    {
        ...
        try
        {
            if (configProp.getProperty("app.enable.logs.in.report").equalsIgnoreCase("false"))
            {
    ...
    private void generateAdditionalInfoReport(List<ISuite> suites) throws Exception
    {
        ...
        if (url == null)
        {
            url = configProp.getProperty("app.url");
        }
    ...
    private void startHtmlPage(PrintWriter out)
    {
        ...
        out.println("<head>");
        out.println("<title>SENSIPLE QA Automation Test Results Summary</title>");
        out.println("</head>");
        out.println("<body><div style=\"margin:0 auto; padding:15px; min-height:450px; min-width: 450px; height:auto;\">"
                ...
                + "<h1 style=\"background-color: #93b874; color: white; text-align: center; font-family: Georgia;\">SENSIPLE QA Automation Report</h1>");
    ...
    {
        out.println("<br/><br/><div align=\"right\"> &copy; <a href=\"http://www.sensiple.com\">Copyrights © Sensiple 2016</a></div>");
